...
func TestPrivateChannelAuthentication(t *testing.T) {
	...
	result, err := client.Authenticate(&PrivateChannelRequest{postParams})
...
func TestPrivateChannelAuthenticationWrongParams(t *testing.T) {
	...
	_, err := client.Authenticate(&PrivateChannelRequest{postParams})
	assert.Error(t, err)
}
...
func TestPresenceChannelAuthentication(t *testing.T) {
	...
	result, err := client.Authenticate(&PresenceChannelRequest{postParams, presenceData})
	assert.Equal(t, expected, string(result))
	assert.NoError(t, err)
}
